...
Book firstBook = new Book("The Great Gatsby", "F. Scott Fitzgerald"
, 180, new DateTime(1925, 4, 10), "Fiction");
Book secondBook = new Book("To Kill a Mockingbird", "Harper Lee"
...
library.AddBook(firstBook); // testing that the library can't add the same book twice
...
workerMember.BorrowBook(firstBook, library);
workerMember.BorrowBook(firstBook, library); // testing that the member can't borrow the same book twice
Book thirdBook = new Book("The Hobbit", "J. R. R. Tolkien"
, 310, new DateTime(1937, 9, 21), "Fiction");
workerMember.BorrowBook(thirdBook, library); // testing that the member can't borrow a book that isn't in the library
...
customerMember.BorrowBook(secondBook, library); // testing that the member can't borrow a book if he isn't registered
workerMember.ReturnBook(secondBook, library); // testing that the member can't return a book if he didn't borrow it
library.DisplayAvailableBooks(); // testing that the library displays available books correctly
                                // after the borrow and return operations were not successfull
library.RegisterMember(customerMember);
customerMember.BorrowBook(firstBook, library); // testing that the member can't borrow a book that isn't available
workerMember.ReturnBook(firstBook, library);
library.DisplayAvailableBooks();
customerMember.BorrowBook(firstBook, library);
